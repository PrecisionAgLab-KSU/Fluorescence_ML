...
pipeline = make_pipeline(StandardScaler(),
    #SVR(kernel='rbf', epsilon=0.5, C=10, gamma = 0.6),
    SVR(kernel='poly', C=300, degree=3)
)
...
# #Plotting
# plt.plot(Ym0,y_out1, 'ro')
# #plt.xlim([0, 6])
# #plt.ylim([0, 6])
# plt.xlabel("Observed N content ($\%$)")
# plt.ylabel("Estimated N content ($\%$)")
# plt.plot([2, 5], [2, 5], 'k:')
# plt.annotate('r = %.2f'%rr_value1, xy=(2.05, 4.9))#round off upto 3decimals
# plt.annotate('RMSE = %.2f'%rmse_value1, xy=(2.05, 4.6))
# plt.annotate('MAE = %.2f'%mae_value1, xy=(2.05, 4.3))
# matplotlib.rcParams.update({'font.size': 16})
# plt.yticks(np.arange(2, 5.2, .5))
# plt.xticks(np.arange(2, 5.2, .5))
# plt.gca().set_aspect('equal', adjustable='box')
# plt.savefig('NTrain_scatter.png',bbox_inches="tight",dpi=450)
# plt.show()
...
plt.savefig('Regression_N_V6_SVR.png',bbox_inches="tight",dpi=450)
plt.show()
